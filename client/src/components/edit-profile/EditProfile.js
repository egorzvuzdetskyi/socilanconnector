...
class EditProfile extends Component {
    ...
    componentWillReceiveProps = (nextProps) => {
        ...
        if (nextProps.profile.profile) {
            const profile = this.preparePropsForDisplaying(nextProps.profile.profile);

            this.setState(profile);
        }
    };

    preparePropsForDisplaying = props => {
        const profile = props;
        ...
        Object.keys(profile).forEach(key => {
            ...
            profile[key] = this.setEmptyStringIfFieldIsEmpty(profile[key])
        });

        if(profile.social) {
            Object.keys(profile.social).forEach(key => {
                profile[key] = this.setEmptyStringIfFieldIsEmpty(profile.social[key])
            });
        }
    ...
    setEmptyStringIfFieldIsEmpty = (field) => {
        if (isEmpty(field))
            return '';
        else
            return field
